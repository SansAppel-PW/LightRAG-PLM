...
class PLMMilvus:
    ...
    def __open__(self):
        ...
        try:
            self.conn = MilvusClient(
                uri=rep_settings.MILVUS_URI.get_secret_value(),
                token=rep_settings.MILVUS_TOKEN.get_secret_value()
            )
        except Exception:
            logging.exception(
                "Fail to connect %s " % rep_settings.MILVUS_HOST)
